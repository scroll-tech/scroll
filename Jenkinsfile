        ...
        stage('Parallel Test') {
            parallel{
                ...
                stage('Test database package') {
                    steps {
                        sh "cd database && go test -v -race -coverprofile=coverage.db.txt -covermode=atomic \$(go list ./... | grep -v 'database\\|common\\|l1\\|l2\\|coordinator')"
                    }
                }
                stage('Test bridge package') {
                    steps {
                        sh "cd bridge && go test -v -race -coverprofile=coverage.bridge.txt -covermode=atomic \$(go list ./... | grep -v 'database\\|common\\|l1\\|l2\\|coordinator')"
                    }
                }
                stage('Test coordinator package') {
                    steps {
                        sh "cd coordinator && go test -v -race -coverprofile=coverage.coordinator.txt -covermode=atomic \$(go list ./... | grep -v 'database\\|common\\|l1\\|l2\\|coordinator')"
                    }
                }
