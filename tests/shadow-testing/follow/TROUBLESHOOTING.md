...
- **Symptom 1**: `04-prover-up.sh --docker` (or `30-canary-upgrade.sh --docker-provers`) reports "prover container shadow-prover-N failed to start"; `docker logs` shows `Error: No such file or directory (os error 2)` at `prover.rs` `File::open`.
- **Cause 1**: the per-GPU `work_dir` was built as `${SCRIPT_DIR}/../.work/prover-N`. dockerd path-cleans bind-mount targets, but the prover opens its config by the **literal** path — inside the container the `lib/..` component cannot resolve. Bare metal never noticed. Fixed 2026-08-31: the script canonicalizes `work_dir` before generating configs and mounts.
