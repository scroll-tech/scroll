...
func setupCoordinator(t *testing.T, rollersPerSession uint8, wsURL string) *http.Server {
	db := setupDB(t)
	conf := config.Config{
	...
	assert.NoError(t, err)
	cron.NewCollector(context.Background(), db, &conf)
...
func testHandshake(t *testing.T) {
	...
	wsURL := "ws://" + randomURL()
	handler := setupCoordinator(t, 1, wsURL)
	defer func() {
