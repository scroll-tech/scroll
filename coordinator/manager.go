...
func (m *Manager) handleZkProof(pk string, msg *message.ProofDetail) error {
	...
	if types.RollerProveStatus(tmpSubmissionInfo.ProvingStatus) == types.RollerProofValid {
		...
		log.Warn(
			...
			"roller name", tmpSubmissionInfo.RollerName,
			"roller pk", tmpSubmissionInfo.RollerPublicKey,
			"prove type", tmpSubmissionInfo.ProveType,
			"proof id", msg.ID,
		)
		return nil
	}

	log.Info("handling zk proof", "proof id", msg.ID, "roller name", tmpSubmissionInfo.RollerName, "roller pk",
		tmpSubmissionInfo.RollerPublicKey, "prove type", tmpSubmissionInfo.ProveType, "proof time", proofTimeSec)
	...
	if msg.Status != message.StatusOk {
		...
		m.updateMetricRollerProofsGeneratedFailedTimeTimer(tmpSubmissionInfo.RollerPublicKey, proofTime)
		log.Info(
	...
	if verifyErr != nil {
		...
		success = false
		log.Error("Failed to verify zk proof", "proof id", msg.ID, "roller name", tmpSubmissionInfo.RollerName,
			"roller pk", tmpSubmissionInfo.RollerPublicKey, "prove type", msg.Type, "proof time", proofTimeSec, "error", verifyErr)
	...
	if success {
		...
		m.updateMetricRollerProofsVerifiedSuccessTimeTimer(tmpSubmissionInfo.RollerPublicKey, proofTime)
		log.Info("proof verified by coordinator success", "proof id", msg.ID, "roller name", tmpSubmissionInfo.RollerName,
			"roller pk", tmpSubmissionInfo.RollerPublicKey, "prove type", msg.Type, "proof time", proofTimeSec)
	} else {
		coordinatorProofsVerifiedFailedTimeTimer.Update(proofTime)
		m.updateMetricRollerProofsVerifiedFailedTimeTimer(tmpSubmissionInfo.RollerPublicKey, proofTime)
		log.Info("proof verified by coordinator failed", "proof id", msg.ID, "roller name", tmpSubmissionInfo.RollerName,
			"roller pk", tmpSubmissionInfo.RollerPublicKey, "prove type", msg.Type, "proof time", proofTimeSec, "error", verifyErr)
	}
	return nil
}
