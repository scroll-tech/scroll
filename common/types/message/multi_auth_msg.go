...
// MultiAuthMsg is the first message exchanged from the Prover to the Sequencer.
// It effectively acts as a registration, and makes the Prover identification
// known to the Sequencer.
type MultiAuthMsg struct {
	// Message fields
	Identity *MultiIdentity `json:"message"`
	// Prover signature
	Signature string `json:"signature"`
}

// AcceptVersion contains hard_fork_name and prover_version
type AcceptVersion struct {
	ProverVersion string `json:"prover_version"`
	HardForkName  string `json:"hard_fork_name"`
}

// MultiIdentity contains all the fields to be signed by the prover.
type MultiIdentity struct {
	// ProverName the prover name
	ProverName string `json:"prover_name"`
	// Challenge unique challenge generated by manager
	Challenge string `json:"challenge"`
	// AcceptVersions accept versions
	AcceptVersions []AcceptVersion `json:"accept_versions"`
}

// SignWithKey auth message with private key and set public key in auth message's Identity
func (a *MultiAuthMsg) SignWithKey(priv *ecdsa.PrivateKey) error {
...
func (a *MultiAuthMsg) Verify() (bool, error) {
	hash, err := a.Identity.Hash()
	if err != nil {
		return false, err
	}
...
func (a *MultiAuthMsg) PublicKey() (string, error) {
	hash, err := a.Identity.Hash()
	if err != nil {
		return "", err
	}
...
func (i *MultiIdentity) Hash() ([]byte, error) {
