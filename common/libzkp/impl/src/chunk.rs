...
use libc::c_char;
use prover_v4::{zkevm::Verifier as VerifierLoVersion, ChunkProof as ChunkProofLoVersion};
use prover_v5::{
...
pub unsafe extern "C" fn init_chunk_verifier(
    ...
    v5_assets_dir: *const c_char,
) {
    init_env_and_log("ffi_chunk_verify");

    let params_dir = c_char_to_str(params_dir);
    let v4_assets_dir = c_char_to_str(v4_assets_dir);
    let v5_assets_dir = c_char_to_str(v5_assets_dir);

    // TODO: add a settings in scroll-prover.
    env::set_var("SCROLL_PROVER_ASSETS_DIR", v4_assets_dir);
    let verifier_lo = VerifierLoVersion::from_dirs(params_dir, v4_assets_dir);
    env::set_var("SCROLL_PROVER_ASSETS_DIR", v5_assets_dir);
    let verifier_hi = VerifierHiVersion::from_dirs(params_dir, v5_assets_dir);
