...
func InitDB(config *Config) (*gorm.DB, error) {
	...
	sqlDB.SetConnMaxLifetime(config.MaxLifetime != 0 ? config.MaxLifetime : time.Minute * 10)
	sqlDB.SetConnMaxIdleTime(config.MaxIdleTime != 0 ? config.MaxIdleTime : time.Minute * 5)
