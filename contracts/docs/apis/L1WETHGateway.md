...
The `L1WETHGateway` contract facilitates the deposit of WETH tokens on Layer 1 and finalizes the withdrawal of WETH from Layer 2.

*Unlike other gateways, the deposited WETH tokens are not stored within this gateway. Instead, they are initially unwrapped into Ether, which is then forwarded to the `L1ScrollMessenger` contract. Upon finalizing the withdrawal process, the Ether is transferred from the `L1ScrollMessenger`, wrapped back into WETH, and subsequently sent to the recipient.*
