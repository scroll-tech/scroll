...
contract WithdrawTrieVerifierTest is DSTestPlus {
    ...
    function testMerkleProof() public {
        ...
        for(uint i = 0; i < 4; i++) {
            require(WithdrawTrieVerifier.verifyMerkleProof(roots[i], hashes[i], nonces[i] ,  proofs[i]) == true, "WithdrawTrieVerifier: verifyMerkleProof failed");
        }
    }
}
