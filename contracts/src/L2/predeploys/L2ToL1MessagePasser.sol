...
contract L2ToL1MessagePasser is AppendOnlyMerkleTree {
  ...
  event PassMessage(uint256 index, bytes32 messageHash);
  ...
  function passMessageToL1(bytes32 _messageHash) external returns (bytes32) {
    ...
    emit PassMessage(_currentNonce, _messageHash);
