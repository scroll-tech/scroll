...
async function main() {
    ...
    // initialize L2MessageQueue
    const L2MessageQueue = await ethers.getContractAt(
        "L2MessageQueue",
        addressFile_l1.get("L2MessageQueue"),
        deployer
    );
    ...
    // initialize L2TxFeeVault
    const L2TxFeeVault = await ethers.getContractAt(
        "L2TxFeeVault",
        addressFile_l1.get("L2TxFeeVault"),
        deployer
    );
    ...
    const L1GasPriceOracle = await ethers.getContractAt(
        "L1GasPriceOracle",
        addressFile_l1.get("L1GasPriceOracle"),
        deployer
    );
    const L2_WHITELIST_ADDR = addressFile_l2.get("Whitelist")
