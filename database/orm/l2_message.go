...
func (m *layer2MessageOrm) GetL2MessagesByStatusUpToHeight(status MsgStatus, height *big.Int) ([]*L2Message, error) {
	rows, err := m.db.Queryx(`SELECT nonce, msg_hash, height, sender, target, value, fee, gas_limit, deadline, calldata, layer2_hash FROM l2_message WHERE status = $1 AND height <= $2 ORDER BY nonce ASC;`, status, height.Int64())
	if err != nil {
		return nil, err
	}
