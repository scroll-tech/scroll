...
func testCreateNewRelayer(t *testing.T) {
	...
	relayer, err := NewLayer2Relayer(context.Background(), l1Cli, db, cfg.L2Config.RelayerConfig, false, nil)
	assert.NoError(t, err)
...
func testL2RelayerProcessPendingBatches(t *testing.T) {
	...
	l2Cfg := cfg.L2Config
	relayer, err := NewLayer2Relayer(context.Background(), l1Cli, db, l2Cfg.RelayerConfig, false, nil)
	assert.NoError(t, err)
...
func testL2RelayerCommitConfirm(t *testing.T) {
	...
	defer cancel()
	l2Relayer, err := NewLayer2Relayer(ctx, l1Cli, db, l2Cfg.RelayerConfig, false, nil)
	assert.NoError(t, err)
