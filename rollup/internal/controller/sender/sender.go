...
func (s *Sender) SendTransaction(contextID string, target *common.Address, data []byte, blobs []*kzg4844.Blob) (common.Hash, error, uint64) {
	...
	if blobs != nil {
		...
		if s.senderType == types.SenderTypeCommitBatch {
			...
			if err != nil {
				...
				return nil, fmt.Errorf("failed to count pending transactions, err: %w", err)
			}
			if numPendingTransactions >= s.config.MaxPendingBlobTxs {
				return nil, ErrTooManyPendingBlobTxs
			}
		...
		if err != nil {
			...
			return nil, fmt.Errorf("failed to make sidecar for blob transaction, err: %w", err)
		}
	}
	...
	if err != nil {
		...
		return nil, fmt.Errorf("failed to get block number and base fee, err: %w", err)
	}
	...
	if feeData, err = s.getFeeData(target, data, sidecar, baseFee, blobBaseFee); err != nil {
		...
		return nil, fmt.Errorf("failed to get fee data, err: %w", err)
	}
	...
	if err != nil {
		...
		return nil, fmt.Errorf("failed to create signed transaction, err: %w", err)
	}
	...
	if err = s.pendingTransactionOrm.InsertPendingTransaction(s.ctx, contextID, s.getSenderMeta(), signedTx, blockNumber); err != nil {
		...
		return nil, fmt.Errorf("failed to insert transaction, err: %w", err)
	}
	...
	if err := s.client.SendTransaction(s.ctx, signedTx); err != nil {
		...
		if updateErr := s.pendingTransactionOrm.DeleteTransactionByTxHash(s.ctx, signedTx.Hash()); updateErr != nil {
			...
			return nil, fmt.Errorf("failed to delete transaction, err: %w", updateErr)
		}
		...
		return nil, fmt.Errorf("failed to send transaction, err: %w", err)
	}
