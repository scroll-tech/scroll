...
func (l *l1CrossMsgOrm) DeleteL1CrossMsgAfterHeightDBTx(dbTx *sqlx.Tx, height int64) error {
	if _, err := l.db.Exec(`UPDATE cross_message SET deleted_at = $1 WHERE height > $2 AND msg_type = $3;`, time.Now(), height, Layer1Msg); err != nil {
		return err
	}
	return nil
}
