...
func action(ctx *cli.Context) error {
	...
	l1BlocktimeFetcher := cross_msg.NewBlocktimestampFetcher(subCtx, uint(cfg.L1.Confirmation), int(cfg.L1.BlockTime), l1client, db.UpdateL1Blocktimestamp, db.GetL1LatestNoBlocktimestampHeight)
	go l1BlocktimeFetcher.Start()
	defer l1BlocktimeFetcher.Stop()

	l2BlocktimeFetcher := cross_msg.NewBlocktimestampFetcher(subCtx, uint(cfg.L2.Confirmation), int(cfg.L2.BlockTime), l2client, db.UpdateL2Blocktimestamp, db.GetL2LatestNoBlocktimestampHeight)
