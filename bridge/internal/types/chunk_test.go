...
func TestChunkHash(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, "78c839dfc494396c16b40946f32b3f4c3e8c2d4bfd04aefcf235edec474482f8", hash.Hex())
	...
	hash, err = chunk.Hash(0)
	assert.NoError(t, err)
	assert.Equal(t, "aa9e494f72bc6965857856f0fae6916f27b2a6591c714a573b2fab46df03b8ae", hash.Hex())
	...
	hash, err = chunk.Hash(0)
	assert.NoError(t, err)
	assert.Equal(t, "42967825696a129e7a83f082097aca982747480956dcaa448c9296e795c9a91a", hash.Hex())
}
