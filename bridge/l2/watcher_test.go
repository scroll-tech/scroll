...
func testCreateNewWatcherAndStop(t *testing.T) {
	...
	l2cfg := cfg.L2Config
	rc := NewL2WatcherClient(context.Background(), []*ethclient.Client{l2Cli}, l2cfg.Confirmations, l2cfg.L2MessengerAddress, l2cfg.L2MessageQueueAddress, l2db)
	rc.Start()
	defer rc.Stop()
...
func prepareWatcherClient(l2Cli *ethclient.Client, db database.OrmFactory, contractAddr common.Address) *WatcherClient {
	...
	return NewL2WatcherClient(context.Background(), []*ethclient.Client{l2Cli}, confirmations, contractAddr, contractAddr, db)
}
