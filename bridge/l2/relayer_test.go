...
func testCreateNewRelayer(t *testing.T) {
	...
	relayer, err := l2.NewLayer2Relayer(context.Background(), l2Cli, db, cfg.L2Config.RelayerConfig)
	assert.NoError(t, err)
	defer relayer.Stop()
...
func testL2RelayerProcessSaveEvents(t *testing.T) {
	...
	l2Cfg := cfg.L2Config
	relayer, err := l2.NewLayer2Relayer(context.Background(), l2Cli, db, l2Cfg.RelayerConfig)
	assert.NoError(t, err)
	defer relayer.Stop()
