...
func initEnv(t *testing.T) error {
	img = docker.NewImgDB(t, "postgres", "123456", "test", 5435)
	assert.NoError(t, img.Start())
	db, err := NewConnection(&config.DBConfig{
		DriverName: "postgres",
		DSN:        img.Endpoint(),
	})
	if err != nil {
...
func TestOrm_RollupResult(t *testing.T) {
	...
	err = ormRollup.UpdateFinalizeTxHashAndStatus(context.Background(), uint64(templateRollup[1].Number), templateRollup[1].FinalizeTxHash, orm.RollupStatus(templateRollup[1].Status))
	assert.NoError(t, err)
